...
public class SPC_DataDAO {
	...
	public void generateGraph(Pass_VO vo, HttpSession session, HttpServletResponse response,
			HttpServletRequest request) {
		try {
			DecimalFormat df = new DecimalFormat("00.000");
			String column="", disp_query = "";
			int ct=0,ct1=0,ct2=0,ct3=0,ct4=0;
			Connection conMaster = Connection_Util.getConnectionMaster(); 	
			PreparedStatement ps_check = null,ps_vendor =null;
			ResultSet rs_check=null,rs_vendor =null;
			...
			//System.out.println("date from = = " + vo.getDatafor());
			if (vo.getDatafor().equalsIgnoreCase("all")) {
				disp_query = "select top 50 * from baker_vendor_sidd1 where enable=1 and CAST(datetime_sheet as date) between '" + vo.getFromDate() + "' and '" + vo.getToDate() + "' union all "+
							 " select top 50 * from baker_vendor_sidd2 where enable=1 and CAST(datetime_sheet as date) between '" + vo.getFromDate() + "' and '" + vo.getToDate() + "' union all "+
							 " select top 50 * from baker_vendor_jay where enable=1 and CAST(datetime_sheet as date) between '"	+ vo.getFromDate() + "' and '" + vo.getToDate() + "' union all "+
							 " select top 50 * from baker_vendor_aryan where enable=1 and CAST(datetime_sheet as date) between '"	+ vo.getFromDate() + "' and '" + vo.getToDate() + "'";
				/*disp_query = "SELECT TOP 50 * FROM baker_vendor_sidd1 where CAST(datetime_sheet as date) between '"	+ vo.getFromDate() + "' and '" + vo.getToDate()
		 				+ "' and enable=1";*/
		 	} else {
				...
				for(int i=0;i<multVendor.size();i++){
					if(i==0){
						disp_query = "select top 50 * from "+multVendor.get(i).toString()+" where enable=1 and CAST(datetime_sheet as date) between '"+vo.getFromDate()+"' and '"+vo.getToDate()+"'";
					}else{
						disp_query = disp_query + " union all select top 50 * from "+multVendor.get(i).toString()+" where enable=1 and CAST(datetime_sheet as date) between '"+vo.getFromDate()+"' and '"+vo.getToDate()+"'";
					}
				}
		 	}
			
		 /*ps_check = conMaster.prepareStatement(disp_query);
       	 rs_check = ps_check.executeQuery();
       	 while (rs_check.next()) {
       		 System.out.println("val = = " + Double.valueOf(rs_check.getString(column)));
			...
			/*********************************************************************************/
			 ...
			 /*********************************************************************************/
			 /*********************************************************************************/
				 RequestDispatcher rd = request.getRequestDispatcher("/SPC_Report.jsp");
